...
unsigned next_vv_size(unsigned n)
{
  ...
  {
    if (p > (1u << 30))
      die("van-voorhis size too large");
    if (p > (1u << 30))
      die("van-voorhis size too large");
    p = p * p;
  }
...
net_t gen_van_voorhis(unsigned n)
{
  ...
  if (!is_vv)
  {
    ...
    for (unsigned l = 0; l < padded_net.layers; ++l)
      ...
      {
        unsigned a = padded_net.layer[l].pair[k].left,
                 b = padded_net.layer[l].pair[k].right;
        if (a < orig_n && b < orig_n)
          seq_add(&filtered, a, b);
      }
    net_free(&padded_net);
